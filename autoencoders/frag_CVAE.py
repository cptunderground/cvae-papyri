...
def evaluate(losses, autoencoder, dataloader, flatten=True):
    ...
    label_encoder_char.fit(all_labels_frags)

    for inputs, labels, frags in dataloader:
        inputs = inputs.to(DEVICE)
        labels = label_encoder_char.transform([frags])
...
@decorators.timed
def _train_frag_cvae(net, dataloader, test_dataloader, flatten=True, epochs=20):
    print("training")
    validation_losses = []
...
def get_latent_data(net, dataset, count=1000, is_cvae=False):
    ...
    with torch.set_grad_enabled(False):
        ...
        for inputs, labels, frags in dataset_loader:
            ...
            labels = label_encoder_char.transform(frags)
            labels_one_hot = one_hot(labels, labels_length - 1).to(DEVICE)
            if is_cvae:
...
def train_frag_cvae(config: Config, result: Result):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    torch.save(optimal_model[0], f'./{config.root}/frag_CVAE-optimal-{optimal_model[1]}-{config.name_time}.pth')
    config.cvae_frag_path = f'./{config.root}/frag_CVAE-optimal-{optimal_model[1]}-{config.name_time}.pth'
    result.frag_cvae = f'./{config.root}/frag_CVAE-optimal-{optimal_model[1]}-{config.name_time}.pth'

    result.frag_cvae_train_loss = losses_train
    result.frag_cvae_valid_loss = losses_valid
    result.frag_cvae_test_loss = losses_test

    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.plot(losses_train)
    util.utils.create_folder(f"./{config.root}/net_eval/frag_CVAE")
    plt.title("Train Loss - frag_CVAE")
    plt.savefig(f"./{config.root}/net_eval/frag_CVAE/frag_CVAE_loss_train.png")
    ...
    util.utils.create_folder(f"./{config.root}/net_eval/frag_CVAE")
    plt.title("Validation Loss - frag_CVAE")
    plt.savefig(f"./{config.root}/net_eval/frag_CVAE/frag_CVAE_loss_valid.png")
    ...
    util.utils.create_folder(f"./{config.root}/net_eval/frag_CVAE")
    plt.title("Test Loss - frag_CVAE")
    plt.savefig(f"./{config.root}/net_eval/frag_CVAE/frag_CVAE_loss_test.png")
    ...
    plt.legend()
    # plt.yscale("log")
    plt.savefig(f"./{config.root}/net_eval/frag_CVAE/frag_CVAE_loss_all.png")
