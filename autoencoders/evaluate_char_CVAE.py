...
def evaluate(config: Config, result: Result):
    ...
    util.utils.create_folder(f"./{config.root}/net_eval/char_CVAE_eval")
    ...
    char_cvae = char_CVAE(48)
    char_cvae = torch.load(config.cvae_char_path)
    char_cvae.eval()

    all_labels_char = ["alpha", "beta", "chi", "delta", "epsilon", "eta", "gamma", "iota", "kappa", "lambda", "mu",
                       "nu",
    ...
    for image, label_char, label_frag in test_loop:
        ...
        label = label_encoder_char.transform(label_char)

        ae_resnet18.eval()
        with torch.no_grad():
            _enc, _dec = ae_resnet18(image)

            label = one_hot(label, 23).to(device)

            _enc = _enc.view(_enc.size(0), 48)

            mu, logvar = char_cvae.encode(_enc, label)
            _enc_cvae = char_cvae.reparameterize(mu, logvar)
    ...
    plt.savefig(f'./{config.root}/net_eval/char_CVAE_eval/umap_scatter_{config.chars_to_eval}_char.png')
    plt.close()
    ...
    plt.savefig(f'./{config.root}/net_eval/char_CVAE_eval/umap_scatter_{config.chars_to_eval}_frag.png')
    plt.close()
    ...
    plt.savefig(f'./{config.root}/net_eval/char_CVAE_eval/umap_ncluster_{config.chars_to_eval}.png')
    plt.close()
    ...
    plt.savefig(f'./{config.root}/net_eval/char_CVAE_eval/umap_ncomp3_{config.chars_to_eval}.png')
    plt.close()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/umap_char_labels_gist.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/kmeans_char_labels_tab20.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/clusterable_embeddings_umap_char_labels.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/hdbscan_char_labels_1.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/hdbscan_char_labels_2.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/umap_frag_labels_active_passive.png")
    plt.show()
    plt.close()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/umap_kmeans_clustered_frag_labels_active_passive.png")
    plt.show()

    plt.scatter(standard_embedding[:, 0], standard_embedding[:, 1], c=kmeans_labels, s=5, cmap='gist_ncar')
    plt.title("scatter-kmeans-gist")
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/scatter-kmeans-gist.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/umap_clusterable_embeddings_frag_labels.png")
    plt.show()
    ...
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/hdbscan_frag_labels_1.png")
    plt.show()

    plt.scatter(standard_embedding[clustered, 0], standard_embedding[clustered, 1], c=hdbscan_labels[clustered], s=5,
                cmap='gist_ncar')
    plt.title(f"hdbscan_frag_labels_2")
    plt.savefig(f"./{config.root}/net_eval/char_CVAE_eval/hdbscan_frag_labels_2.png")
    plt.show()
